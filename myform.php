<html>
<head>
   <style>

    body {
      background: white;
      font-family: "Lucida Grande", "Lucida Sans Unicode", Verdana, sans-serif;
      font-size: 11px;
      line-height: 18px;
      text-rendering: optimizeLegibility;
      color: #323B41;
      margin: 0;
      padding: 0;
    }
    
    
	#login { 
		position: relative; right: 20%;
		
		}
		 
		 

      #wrapper {
        width: 425px;
        margin: 0 auto 3em;
        position: relative;
      }



        #header {
          text-align: center;
          margin: 3.25em 0 6.5em 0;
        }

          #header h1 {
            font-family: "Gill Sans", "Gill Sans Std", "Gill Sans MT", Georgia, serif;
            font-weight: normal;
            font-size: 15px;
            line-height: 21px;
            letter-spacing: 1px;
            text-transform: uppercase;
            margin: 0;
          }

          #header h1 a, #header h1 a:hover {
            color: #323B41;
            text-decoration: none;
          }

          #description {
            font-size: 10px;
            line-height: 15px;
            color: #A8A8A8;
            text-align: center;
            margin: 0.5em 0 0 0;
          }

          #description a {
            color: #A8A8A8;
            text-decoration: none;
            padding-bottom: 1px;
            border-bottom: 1px solid #C6D8E4;
          }

          #description a:hover {
            border-bottom: none;
          }

          #description div {
            margin-top: 0.5em
          }

          #description ul#pages {
            display: inline;
            text-align: center;
            list-style: none;
            margin-left: 0;
            padding-left: 0;
          }

          #description ul#pages li {
            list-style: none;
            display: inline;
            margin-right: 1.5em;
          }

        #content {
          margin: 0;
        }

          #search-results {
            margin: 0 0 2em 0;
          }

          .permalink {
            position: absolute;
            right: 460px;
            text-align: right;
          }

          .permalink a {
            color: #A8A8A8 !important;
            text-decoration: none;
            border-bottom: none !important;
          }

          .permalink a:hover {
            color: #627480 !important;
          }

          #content p {
            margin: 0 0 12px 0;
          }

          #content a {
            color: #627480;
            text-decoration: none;
            padding-bottom: 1px;
            border-bottom: 1px solid #C6D8E4;
          }

          #content a:hover {
            border-bottom: none;
          }

          #content h2 {
            font-family: "Gill Sans", "Gill Sans Std", "Gill Sans MT", Georgia, serif;
            font-weight: normal;
            font-size: 15px;
            line-height: 21px;
            margin-bottom: 12px;
            
          }

          #content h3 {
            font-family: "Gill Sans", "Gill Sans Std", "Gill Sans MT", Georgia, serif;
            font-weight: normal;
            font-size: 12px;
            line-height: 18px;
            text-transform: uppercase;
            letter-spacing: 1px;
            margin-bottom: 12px;
          }

          #content blockquote {
            margin-right: 0;
            margin-bottom: 12px;
            margin-left: 1em;
            padding-left: 1em;
            border-left: 1px solid #C6D8E4;
          }

          #content ul, #content ol {
            margin-left: 1em;
            padding-left: 1em;
            margin-bottom: 12px;
          }

          #content img {
            max-width: 100%;
          }

          .post {
            margin: 0 0 6.5em 0;
          }

          .post img, .post embed, .post object {
            display: block;
            margin: 0 0 19px 0;
            border: none;
          }

            .text h2 a, .text h2 a:hover, .chat h2 a, .chat h2 a:hover {
              color: #323B41 !important;
              border-bottom: none !important;
            }

            .link h2 a {
              color: #627480;
              text-decoration: none;
              padding-bottom: 1px;
              border-bottom: 1px solid #C6D8E4;
            }

            .link h2 a:hover {
              border-bottom: none;
            }

            .photo, .video, .audio {
              padding: 4px 0 0 0;
            }

            .video embed, .video object {
              min-width: 100%;
            }

            /* This should fix Vimeo embeds, correct width and bottom margin */
            .video iframe {
              width: 100%;
              margin: 0 0 13px 0;
            }

            .audio {
              min-height: 80px;
            }

            .big-quote {
              width: 100% !important;
              font-family: "Gill Sans", "Gill Sans Std", "Gill Sans MT", Georgia, serif !important;
              font-size: 20px !important;
              line-height: 25px !important;
              text-indent: -0.45em !important;
              border-left: none !important;
              margin-left: 0 !important;
              padding-left: 0 !important;
            }

            .big-quote:before {
              content: '“';
            }

            .big-quote:after {
              content: '”';
            }

            .chat ul {
              list-style: none;
              margin-left: 0 !important;
              padding-left: 0 !important;
            }

            .album-art {
              float: left;
              width: 80px;
              height: 80px;
              margin: 0 13px 13px 0;
              -webkit-border-radius: 2px;
              -moz-border-radius: 2px;
              -webkit-box-shadow: 0 1px 3px #A8A8A8;
              -moz-box-shadow: 0 1px 3px #A8A8A8;
              overflow: hidden;
            }

          ol.notes {
            list-style: none;
            margin-left: 0 !important;
            padding-left: 0 !important;
          }

          ol.notes li.note img.avatar {
            display: none;
          }

        #nav {
          text-align: center;
        }

        #nav a {
          color: #627480;
          text-decoration: none;
          padding-bottom: 1px;
          border-bottom: 1px solid #C6D8E4;
        }

        #nav a:hover {
          border-bottom: none;
        }

        #footer {
          color: #A8A8A8;
          font-size: 10px;
          margin: 6.5em 0 0 0;
          text-align: left;
        }

        #footer a {
          color: inherit;
          text-decoration: none;
          padding-bottom: 1px;
          border-bottom: 1px solid #C6D8E4;
        }

        #footer a:hover {
          border-bottom: none;
        }

        #footer form {
          position: absolute;
          bottom: 0;
          right: 0;
        }

        #footer input {
          width: 140px;
          height: 15px;
          border: 1px solid #A8A8A8;
          font-family: "Lucida Grande", "Lucida Sans Unicode", Verdana, sans-serif;
          font-size: 10px;
          line-height: 15px;
          color: #A8A8A8;
        }
        
	.class1 A:link {text-decoration: none;
	color: gray;
	}
        

    </style>
</head>
<body>


<div id="login">
<span class="class1"><br>
	<?php include('./includes/login.inc.php')
	?>
	</span>

</div>

<div id="header">
    <h1>somethinghappyplz</h1>
</div>
<div id="wrapper">
    <div id="maincontent">
      <h2>Something happy for [username] </h2>
      
<form id="feedback" method="post" action="">

<?php 
if ($_POST) {
	include('./includes/nuke_magic_quotes.php');
		}
	//now come the parts of the form the user can FILL IN yay!!
	?>
	
	
	
	<p>
	<label for="name">Link:</label>
	
	<input name="name" id ="name" type="text" class="formbox" style="width:440px">
	</p>
	
	<p>
     <label for="comments">Comment:</label>
                <textarea name="comments" id="comments" style="width:440px" rows ="8"></textarea>
            </p>
             <p>
                <input name="send" id="send" type="submit" value="Send message">
            </p>
	</p>
	<pre>
	<?php if ($_POST) { print_r($_POST); } ?>
	</pre>
	</form>